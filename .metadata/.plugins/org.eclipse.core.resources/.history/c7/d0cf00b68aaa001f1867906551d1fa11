...
void fsm_processing(){
	...
	case MODE4:
		...
		//Khi nhấn nút Mode sẽ chuyển 1 trong 2 trạng thái
		if(isSelectPressed() == 1){
			//Nếu thời gian đèn đỏ = đèn vàng + đèn xanh thì trả về chế độ normal
			...
			//Nếu không sẽ trả chế độ ban đầu và khi nhấn nút MODE sẽ cài đặt lại ở led đỏ
			else{
				MODE = STATE_INIT;
				if(MODE == STATE_INIT && isSelectPressed() == 1){
					MODE = MODE1;
				}
			break;
			}
